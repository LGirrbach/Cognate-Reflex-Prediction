...
def get_settings(trial_num: int, model: str, num_layers: int, hidden_size: int, dropout: float, batch_size: int,
    ...
    return make_settings(
        model=model, name=name, save_path=f"{base_path}/results/multilingual/saved_models",
        epochs=epochs, device=torch.device("cuda:0"), optimizer="adam", batch=batch_size, tau=5, verbose=True,
...
if __name__ == '__main__':
    ...
    prediction_save_path = os.path.join(base_path, "results/multilingual/predictions")
    os.makedirs(prediction_save_path, exist_ok=True)

    # pbar = tqdm(desc="Progress", total=4 * 4 * 5)

    source_cognate_sets_train = []
    source_languages_train = []
    targets_train = []
    target_languages_train = []

    test_datasets = dict()

    for dataset in datasets:
        train_data = read_train_dataset(dataset, level=10)
        source_cognate_sets_train.extend(train_data.source_cognate_sets)
        source_languages_train.extend(train_data.source_languages)
        targets_train.extend(train_data.targets)
        target_languages_train.extend(train_data.target_languages)
        ...
        test_datasets[dataset] = RawDataset(
            source_cognate_sets=test_cognate_sets, source_languages=test_languages_source,
            targets=test_targets, target_languages=test_languages_target
        )

    train_data = RawDataset(
        source_cognate_sets=source_cognate_sets_train, source_languages=source_languages_train,
        targets=targets_train, target_languages=target_languages_train
    )
            ...
            for trial in range(1, 5+1):
                ...
                for dataset_name, dataset in test_datasets.items():
                    predictions = transducer.predict(
                        sources=dataset.source_cognate_sets, source_languages=dataset.source_languages,
                        target_languages=dataset.target_languages
                    )

                    prediction_file_name = f"cognates-model={model_name}-dataset={dataset_name}"
