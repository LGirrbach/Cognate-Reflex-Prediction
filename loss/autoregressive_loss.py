...
def fast_autoregressive_transduction_loss(scores: Tensor, source_lengths: Tensor, target_lengths: Tensor,
                                          insertion_labels: Tensor, substitution_labels: Tensor, deletion_index: int,
                                          device: torch.device, reduction: str = "mean") -> Tensor:
...
def _autoregressive_slow_recursion(probability_matrix: List[List[Tensor]], scores: Tensor, source_index: int,
                                   target_index: int, source_length: int, deletion_index: int, insertion_labels: Tensor,
                                   substitution_labels: Tensor, batch_indices: Tensor, skip_undefined: bool,
                                   neg_inf_vector: Tensor, source_lengths: Tensor):
    probabilities = []
    ...
    for source_index in range(0, source_length + 1):
        for target_index in range(0, target_length):
            ...
            probabilities, _ = _autoregressive_slow_recursion(
                ...
                insertion_labels=insertion_labels, substitution_labels=substitution_labels, batch_indices=batch_indices,
                skip_undefined=True, neg_inf_vector=neg_inf_vector, source_lengths=source_lengths
            )
