...
class BiLSTMEncoder(nn.Module):
    ...
    def __init__(self, input_size: int, hidden_size: int = 128, num_layers: int = 1, dropout: float = 0.0,
        ...
        self.lstm = nn.LSTM(
            ...
            bidirectional=True, dropout=(0.0 if self.num_layers == 1 else self.dropout)
        )

        if self.reduce_dim:
            self.reduce_dim_layer = nn.Linear(2 * self.hidden_size, self.hidden_size)
        else:
            self.reduce_dim_layer = nn.Identity()
    ...
    def forward(self, inputs: torch.Tensor, lengths: torch.Tensor) -> torch.Tensor:
        ...
        # Project down
        encoded = self.reduce_dim_layer(encoded)
